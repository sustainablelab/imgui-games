...
int main(int, char**)
{
...

    {
        ...
        glGetShaderiv(vertexShader, GL_COMPILE_STATUS, &vertex_shader_success);
        assert(vertex_shader_success);
    }
...

    {
        ...
        glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &fragment_shader_success);
        assert(fragment_shader_success);
    }
...

    {
        ...
        glGetProgramiv(shaderProgram, GL_LINK_STATUS, &shader_program_success);
        assert(shader_program_success);
    }
    // Main loop
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
        glViewport(0,0,600,600); // Render on the whole framebuffer
